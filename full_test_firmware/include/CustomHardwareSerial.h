...
#ifndef CustomHardwareSerial_h
#define CustomHardwareSerial_h
...
class CustomHardwareSerial : public Stream {
  protected:
  ...
  public:
    CustomHardwareSerial(uint32_t _rx, uint32_t _tx, uint32_t _rts = NUM_DIGITAL_PINS, uint32_t _cts = NUM_DIGITAL_PINS);
    CustomHardwareSerial(PinName _rx, PinName _tx, PinName _rts = NC, PinName _cts = NC);
    CustomHardwareSerial(void *peripheral, HalfDuplexMode_t halfDuplex = HALF_DUPLEX_DISABLED);
    CustomHardwareSerial(uint32_t _rxtx);
    CustomHardwareSerial(PinName _rxtx);
    ...
    static void attach_custom_tx_callback(void (*callback)(serial_t *));
  ...
  private:
    ...
    inline static void (*_custom_tx_callback)(serial_t *);
};
